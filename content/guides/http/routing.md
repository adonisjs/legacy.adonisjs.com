...
HTTP routes are the static or the dynamic URLs of your application. For example: 

- URLs like `/`, `/about` or `/contact` are the static routes.
- Whereas, URLs like `/posts/1` are dynamic as they accept the post id and serve the post at runtime.

Routes in AdonisJS are defined inside the `start/routes.ts` using the **Route** provider. Give it a try by defining the following routes.
...
Route.get('/posts/:id', async ({ params }) => {
  return `You are viewing the post with id ${params.id}`
})
```
...
- A typical routes needs a minimum of two arguments. First is the route pattern and second is the route handler.
- The handler is invoked anytime AdonisJS receives a request for the matching pattern.
- Returning a value from the route handler is one of the many ways to send a response for the request.
- The `start/routes.ts` is imported automatically by AdonisJS. You can inspect the `.adonisrc.json` file to get a list of preloaded files.
...
Make sure to start the HTTP server by running the `node ace serve --watch` command and then visit the registered URLs inside a browser.

:::

Make sure to start the HTTP server by running the `node ace serve --watch` command and then visit the registered URLs inside a browser.

Any other URLs except the registered one's will result in a **404 (Page not found)** error.
...
Now, if you will visit the [`http://localhost:3333/posts/1`](http://localhost:3333/posts/1), you will see the following message.
...
::video{url="https://res.cloudinary.com/adonis-js/image/upload/q_80/v1582021461/adonisjs.com/dynamic-urls.mp4" controls=true}
...
::video{url="https://res.cloudinary.com/adonis-js/image/upload/q_80/v1582259620/adonisjs.com/route-middleware.mp4" controls=true}
